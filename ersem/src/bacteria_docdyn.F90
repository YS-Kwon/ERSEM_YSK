...
module ersem_bacteria_docdyn
   ...
   type,extends(type_ersem_pelagic_base),public :: type_ersem_bacteria_docdyn
      ! Variables
      type (type_state_variable_id) :: id_O3c, id_O2o, id_TA, id_N3n,id_O6c   !!YSK added O6c
      ...
      type (type_diagnostic_variable_id) :: id_mnp,id_decomp

      ! Parameters
      integer  :: nRP
      integer  :: iswBlimX, iswBCH4, iswPCH4   !!YSK added
      ...
      real(rk) :: kbch4,fpch4,kp   !!YSK added
      integer  :: denit
...
!BOC
      ...
      call self%register_state_dependency(self%id_O6c,'O6c','mmol C/m^3','methane')   !!YSK added
      ...
      !!!YSK added for methane
      call self%register_diagnostic_variable(self%id_mnp,'mnp','umol C/m^3/d','methane Mnp prod rate')
      call self%register_diagnostic_variable(self%id_decomp,'decomp','umol C/m^3/d','methane bacterial prod rate')

      !!YSK added for methant production by bacteria
      call self%get_parameter(self%iswBCH4,'iswBCH4','','Bacteria mediated production switch (0: off, 1: on)',default=1)
      call self%get_parameter(self%kbch4,'kbch4','m^3/umol C/d','Methanogenesis per unit of bacterial respiration')
      !!YSK added for methant production by methylphosphonate 
      call self%get_parameter(self%iswPCH4,'iswPCH4','','MPn mediated production switch (0: off, 1: on)',default=1)
      call self%get_parameter(self%fpch4,'fpch4','','Fraction of DOP that is methylphosphonate (0.1-0.3 based on Karl et al 2008')
      call self%get_parameter(self%kp,'kp','uM','Half saturation constant of phosphate for use of methylphosphonate (0.1-0.3 based on Karl et al 2008')
   ...
   ! !LOCAL VARIABLES:
      ...
      real(rk) :: N1pP,N4nP,R1c,R1cP,R1pP,R1nP,R2c,N3n,N6,O2o,O6c,BCH4,PCH4
      real(rk) :: qpB1c,qnB1c
      real(rk) :: etB1,eO2B1
...
!!!!!!!!!!!YSK added for methane production by bacteria mediated process
         if (self%iswBCH4 .eq. 1) then
           Bch4 = fB1O3c * self%kbch4
           _SET_DIAGNOSTIC_(self%id_decomp,BCH4)
           _SET_ODE_ (self%id_O6c,  Bch4)
           _SET_ODE_ (self%id_O3c, -Bch4/12.01)
         end if
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!YSK added for methane production by bacteria mediated process
         If (self%iswPCH4 .eq. 1) then
            Pch4 =  sugB1*R1pP*self%fpch4 *self%kp/(self%kp+N1pP) 
            _SET_DIAGNOSTIC_(self%id_mnp, Pch4)
            _SET_ODE_ (self%id_O6c, Pch4)
            _SET_DIAGNOSTIC_(self%id_fR1B1c, sugB1*R1cP-Pch4)
         end if
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
