...
!  Meteorological forcing variables
   ...
   type (type_scalar_input), public, target            :: seaice   !!YSK added
...
!  Read namelist variables from file.
   ...
   call seaice%configure(method=2, path=meteo_file, index=7)
...
!BOC
   ...
   !!YSK added sea ice!!!!!!!!!!!!
   call branch%get(seaice, 'seaice', 'sea ice concentration', '1', &
                minimum=0._rk, maximum=1._rk, default=0._rk)
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!      
...
!  The short wave radiation
   ...
   if (fluxes_method /= 0) then
      ...
      call register_input(seaice)    !!YSK added
...
!     Optionally calculate surface shortwave radiation from location, time, cloud cover.
      if (I_0%method .eq. 3) then
         ...
         I_0%value = I_0%scale_factor*shortwave_radiation(zenith_angle,yearday,dlon,dlat,cloud%value,seaice%value)
      end if
...
! !LOCAL VARIABLES:
   ...
   REALTYPE, save            :: h1,tx1,ty1,cloud1,seaice1
   REALTYPE, save            :: h2,tx2,ty2,cloud2,seaice2
   integer, save             :: line
   integer                   :: rc
#endif
...
!  Do the time interpolation
   ...
   seaice%value = seaice1 + t*alpha(6)
#else
   if (sst .lt. 100.) then
      tw  = sst
      tw_k= sst+KELVIN
...
!BOC
   ...
   LEVEL2 'cloud',cloud
   LEVEL2 'seaice',seaice    !!YSK added
