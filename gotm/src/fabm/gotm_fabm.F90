...
!  Observation indices (from obs_0d, obs_1d) for pelagic and benthic state variables.
   ...
   type (type_horizontal_variable_id),save :: lon_id,lat_id,windspeed_id,par_sf_id,cloud_id,taub_id,swr_sf_id,seaice_id   !YSK
...
!BOC
   ...
   if (fabm_calc) then
      ...
      seaice_id    = model%get_horizontal_variable_id(standard_variables%ice_area_fraction)
...
! !INTERFACE:
   subroutine set_env_gotm_fabm(latitude,longitude,dt_,w_adv_method_,w_adv_ctr_,temp,salt_,rho_,nuh_,h_,w_, &
                                bioshade_,I_0_,cloud,seaice,taub,wnd,precip_,evap_,z_,A_,g1_,g2_, &
...
! !INPUT PARAMETERS:
   ...
   REALTYPE, intent(in),target :: I_0_,cloud,wnd,precip_,evap_,taub,seaice
...
!BOC
   ...
   call model%link_horizontal_data(seaice_id,   seaice)
